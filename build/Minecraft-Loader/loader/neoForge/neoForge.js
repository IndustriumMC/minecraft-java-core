...
class NeoForgeMC extends events_1.EventEmitter {
    ...
    async downloadInstaller(Loader) {
        ...
        if (versionInfo.isWeeklySnapshot) {
            ...
            versions = metaData.versions.filter((v) => v.toLowerCase().includes(snapshotId) ||
        ...
        // Handle new snapshot format (e.g., "26.1-snapshot-1")
            ...
            versions = metaData.versions.filter((v) => v.startsWith(shortVersion) || v.includes('snapshot'));
            oldAPI = false;
        }
        ...
        // Handle standard releases (e.g., "1.20.4", "1.21.3")
            ...
            versions = legacyMetaData.versions.filter((v) => v.includes(`${this.options.loader.version}-`));
