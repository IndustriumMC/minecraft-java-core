...
class Microsoft {
    ...
    async getAccount(oauth2) {
        ...
        if (!mcstore.error) {
            ...
            hasMinecraftEntitlement = items.some((item) => item.name === 'game_minecraft' || item.name === 'product_minecraft');
        }
        if (!hasMinecraftEntitlement) {
            const requestId = typeof crypto_1.default.randomUUID === 'function'
        ...
        let profile = await this.getProfileWithRetry({ access_token: minecraftAccessToken }, 3);
    ...
    async getProfileWithRetry(mcLogin, attempts = 3) {
        let lastResult = { error: 'PROFILE_UNAVAILABLE' };
