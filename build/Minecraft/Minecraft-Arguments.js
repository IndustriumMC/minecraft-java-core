...
class MinecraftArguments {
    ...
    async GetClassPath(versionJson, loaderJson) {
        ...
        for (const dep of combinedLibraries) {
            ...
            const version = semver_1.default.valid(semver_1.default.coerce(parts.version.split('@')[0]));
            ...
            if (version && current.version && semver_1.default.gt(version, current.version) && (isSupportedVersion && isWindows)) {
                map.set(resolvedPath, { ...dep, resolvedPath, version });
            }
        }
