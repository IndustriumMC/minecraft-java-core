...
export default class CustomLoader extends EventEmitter {
	 ...
	 */
	public async downloadAndInstall(): Promise<LoaderResult> {
		...
		const exitCode = await new Promise<number>((resolve) => {
			const proc = spawn(
				config.javaPath,
				['-jar', installerPath, '--installClient', mcRoot],
				{ cwd: mcRoot }
			);
		...
		const versionMatch =
			versionCandidates.find(candidate => candidate.isNew) ??
			versionCandidates
